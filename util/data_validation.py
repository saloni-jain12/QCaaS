...
# pattern = re.compile('^([XY])()([0-9])$')
# pattern = re.compile(r"[x-yX-Y]+\(\d\d\),", re.IGNORECASE)

mode_list = ["verbatim", "simulation", "echo"]

def data_validate(job_string, mode):
    # check input string should be comma separated
    # if detect(job_string, whitelist=[',']) != ',':
    #     return "Invalid job details, should be in \"X(90), Y(180), X(90)\" format"

    # print('Match ---------- ', pattern.match(job_string))
    ...
    for job in job_strings:
        ...
        if (not 0 <= int(angle) <= 360):
            return "Invalid job details, angle should be in 0-360 range"

    ## Validation for mode
    if not mode.lower() in mode_list:
        return "Invalid mode details, Allowed modes - Verbatim, Simulation and Echo"


    return "Correct job details"